-- ilm.lua
-- ILM-specific appendix handling for Typst books
--
-- ILM doesn't have parts support or a show rule for appendices.
-- Instead, ilm expects appendix content as a parameter to the main function.
...
local header_filter = {
  Header = function(el)
    ...
    -- Handle parts: ilm doesn't have parts, emit as unnumbered heading
    if bookItemType == "part" then
      return pandoc.RawBlock('typst',
        '#heading(level: 1, numbering: none)[' .. pandoc.utils.stringify(el.content) .. ']')
    end

    -- Handle appendices: manually set up appendix numbering (ilm style)
    if bookItemType == "appendix" then
      if file.bookItemNumber == 1 or file.bookItemNumber == nil then
        ...
        -- Reset heading counter and set up A.1.1 numbering format (ilm convention)
        -- ilm uses: counter(heading).update(0) and numbering format "A.1.1."
